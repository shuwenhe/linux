...
\chapter{gdb调试器}
使用 gdb 调试 C++ 代码的基本步骤如下：\\
1.编译 C++ 代码时需要加上 -g（或 -ggdb）选项，以便生成调试信息。例如：\\
g++ decode.cpp -g -o decode -O2 -std=c++14\\
2.启动 gdb，指定要调试的可执行文件，例如：\\
gdb decode\\
3.在gdb中设置断点，例如：\\
break main\\
这将在 main 函数的第一条语句处设置断点。\\
4.运行程序，例如：\\
run\\
5.在程序运行到断点处时，可以使用 gdb 的各种命令进行调试，例如：\\
next（简写为 n）：执行下一行语句，不会进入函数内部。\\
step（简写为 s）：执行下一行语句，如果是函数调用，则进入函数内部。\\
print（简写为 p）：打印变量的值。\\
...
>>>>>>> 49564058b9e8e8628587a09e2b41b8e5b953b477
